...
class BrowserProfile {
	...
	public function browser() {
		...
		$vars['OS']		= $this->osDetect( $ua );
		$vars['Browser']	= $this->browserDetect( $ua );
	...
	private function browserDetect( $ua ) {
		$br		= 
		array( 'Browser' => '' , 'Version' => '' );
		
		$br['Browser']= 
		$this->matchIn( $ua, array(
			'Edge'			=> 'EdgeHTML',
		...
		if ( preg_match_all( 
			...
			if ( $i != 1 ) {
				if ( strripos( $ua, "Version" ) < 
					strripos($ua, $br['Browser'][1] ) ) {
					$vars['Version'] = $matches['version'][1];
				} else {
					$vars['Version'] = $matches['version'][0];
				}
			} else {
				$br['Version'] = $matches['version'][0];
			}
			if ( 
				null == $matches['Version'] || 
				"" == $matches['Version'] 
			) {
				$br['Version'] = "unknown"; 
			}
	...
	private function osDetect( $ua ) {
		...
		$this->matchIn( $ua, array(
			...
			'Android'	=> 'android',
			'iOS'		=> 'ios',
			'Mac'		=> 'macintosh|mac os x|mac_powerpc',
			'Linux'		=> 'linux',
		...
		switch( $os[0] ) {
			case 'Windows':
				$v = $this->matchIn( $ua, array(
					'10'		=> 'NT 10.0',
					'10 Build 9879'	=> 'NT 6.4',
					'8.1'		=> 'NT 6.3',
					'8'		=> 'NT 6.2',
					'7'		=> 'NT 6.1',
					'Vista'		=> 'NT 6.0',
	...
	private function matchIn( $ua, $labels, $default ) {
